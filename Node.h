...
class binaryoperation :public base {  // 双目运算符
 ...
 public:
    // 构造函数中通过赋值得到该节点的上一级结点的名称以及运算符并存储起来，以便后来的计算
    binaryoperation(std::string a, std::string b,
        std::string c, std::map <std::string, base*> save) {
        input.push_back(save[a]);
...
class COND :public base {
 ...
 public:
    //存储该节点的上一级结点
    COND(std::string a, std::string b, std::string c,
        std::string d, std::map <std::string, base*> save) {
        input.push_back(save[a]);
        input.push_back(save[b]);
