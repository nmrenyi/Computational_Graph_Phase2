...
- [**OOP大作业：计算图——功能展示**](#OOP%E5%A4%A7%E4%BD%9C%E4%B8%9A%E8%AE%A1%E7%AE%97%E5%9B%BE%E5%8A%9F%E8%83%BD%E5%B1%95%E7%A4%BA)
    - [**组员**](#%E7%BB%84%E5%91%98)
...
第一行输入5个字串,其中第五个字符"3"表示将Tensor经过Reshape变换成的维数，如下表示a为将b转换成3维的Tensor：
   ...
   + 输入样例
    ...
    + 输出样例
        ...
        ```
        + 提示：
            第136行构建的a是一个2个维度、2*4*的Tensor,140行表示a其转换成一个3个维度、2*2*2的Tensor,142行表示将a转换成一个2个维度、2*4的Tensor，145、146行即为输出。
5. Concat功能在_Tensor.h和Tensor.cpp中给出，用以将两个符合concat要求(详见代码及注释)的Tensor按照不同方式(以不同维数为轴)合并成一个Tensor。Concat类结点的输入在第二阶段中进行，输入规范如下：  
输入6个字符串，第4、5个为要Concat的两个Tensor的名称，第6个为Concat的方式，即第六个字串为n，则在第n+1个维度上将两个矩阵合并。如：
    ```
    a = CONCAT b c 0
    ```
    为按第一个维度合并b、c两个Tensor，具体效果见示例代码。  
   + 输入样例
        ```
        2
        a C 2
    ...
    + 输出样例
        ...
        ```
        + 提示： 
            构建a、b均为2个维度、2*2的Tensor，c表示在第1个（0+1）维度上将两个Tensor合并（即在第1个中括号内将两个Tensor的数据合并），d表示在第2个（1+1）维度上将两个Tensor合并（即在第2个中括号内将两个Tensor的数据合并）
1. 实现了运算图的session功能，在EVAL时候输入0，跳转到session功能（为了兼容原来的输入），SES命令跳转或新建session，原来的EVAL可以继续使用，RUN相当于无参数的EVAL，输入EOF中止输入。
    + 输入样例
        ```
        2
        x V 1.0
